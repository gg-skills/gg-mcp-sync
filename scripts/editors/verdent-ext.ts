/**
 * @fileoverview MCP editor adapter for Verdent; manages MCP server configuration for that VSCode extension.
 *
...
export const verdentExtAdapter: EditorAdapter = {
  id: "verdent-ext",
  name: "Verdent (VSCode Ext)",
  type: "vscode-ext",
  projectConfig: undefined, // UI-based configuration
  globalConfig: GLOBAL_CONFIG,
  format: "mcpServers",

  /**
   * Check if Verdent is installed by looking for the config directory.
   */
   ...
   * Verdent only supports global configuration.
   */
  async readConfig(scope: "project" | "global"): Promise<McpConfigFile | null> {
    // Verdent only supports global scope
    if (scope === "project") {
      return {
        path: "",
   ...
   * Only supports global scope (Verdent uses UI for project config).
   */
  async writeConfig(
    ...
    if (scope === "project") {
      return {
        ...
        proposedContent: "",
        diff: "",
        errors: ["Verdent uses UI-based configuration for project scope. Only global scope is supported."],
        warnings: [],
      };
    }
