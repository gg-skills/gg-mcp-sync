/**
 * @fileoverview MCP editor adapter for Roo Code; manages MCP server configuration for that VSCode extension.
 *
...
function getConfigDir(scope: "project" | "global"): string {
  if (scope === "project") {
    return resolvePath(".roo");
  }
  return resolvePath("~/Library/Application Support/Code/User/globalStorage/rooveterinaryinc.roo-cline/settings");
}
...
export const rooCodeExtAdapter: EditorAdapter = {
  id: "roo-code-ext",
  name: "Roo Code (VSCode Ext)",
  type: "vscode-ext",
  ...
  /**
   * Check if Roo Code is installed by looking for the config directory.
   */
